...
roslaunch jackal_gazebo $GZ_LAUNCH_FILE config:=roas points_to_scan:=true world_name:=$SF_DIR/worlds/$GZ_WORLD_FILE x_pos:=-9.0 y_pos:=7.7 z_pos:=1.0 yaw:=0
	# For using lms1xx 2D lidar instead of os1-xx & roas.urdf.xacro  -> config:=front_laser
	#roslaunch jackal_gazebo jackal_world.launch config:=front_laser world_name:=$SF_DIR/worlds/$GZ_WORLD_FILE x_pos:=-9.0 y_pos:=7.7 z_pos:=1.0 yaw:=0
